...
7. Configures encrypted Rancher backup to S3 (optional, requires Vault + S3)
...
| `RANCHER_VERSION` | Rancher Helm chart version | `v2.14.1` |
...
`deploy.sh` Step 7 configures automated encrypted daily backups to S3. When `BACKUP_S3_ENDPOINT`
is set and Vault authentication is available, the script installs the rancher-backup operator
inside the k3k cluster and creates scheduled Backup resources. Credentials are fetched from Vault
at runtime — no credentials are stored in the cluster.

Backups are encrypted with AES-256-CBC. Vault paths:

- S3 credentials: `kv/services/backup/rancher-backup` (access key, secret key)
- Encryption key: `kv/services/backup/rancher-backup-encryption` (base64-encoded 32-byte key)

If backup is not configured during deploy, you can configure it manually after:

```bash
./rancher-backup.sh \
    --backup-s3-endpoint backup.example.com \
    --backup-s3-bucket rancher-backups \
    --backup-s3-ca-path /path/to/backup-ca.pem
```

### Manual Restoration

Full etcd-level restore using the rancher-backup operator preserves users, RBAC, fleet state,
cluster registrations, settings, and tokens. Works with any Rancher cluster (k3k, RKE2, K3s, Docker).
...
This is the public, read-only copy of the project. Each push is a single
orphan commit — there is no upstream history to mine. A local sync step
removes secrets and developer tooling, scrubs PII (private domain names,
internal IPs, absolute home paths, contributor identifiers), runs a hard
verification gate, then force-pushes a single commit to this remote.

`shellcheck` and `yamllint` gate the publish path; run them locally before
publishing.
...
```text
rancher-k3k/
├── .gitlab-ci.yml           # Legacy CI pipeline (kept for reference)
├── .yamllint.yaml           # Linting rules for YAML manifests
├── deploy.sh                # Automated deployment script (8 steps, with backup)
...
│   │   └── k3k-stuck-k8s-upgrade.md       # Recovery for stuck upgrades via system-upgrade-controller
...
│   └── destroy-test.sh
└── scripts/
    ├── sync-to-github.sh    # Local sync to GitHub mirror (orphan commit + scrub)
    └── sync-to-public.sh    # Local sync to staging public-repo subdir
```
