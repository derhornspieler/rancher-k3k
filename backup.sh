...
HOSTNAME=$(kubectl get ingress rancher-ingress -n "$K3K_NS" \
...
# Service
kubectl get svc rancher-traefik -n "$K3K_NS" -o yaml > "$BACKUP_DIR/host-service.yaml" 2>/dev/null || true

# Ingress
kubectl get ingress rancher-ingress -n "$K3K_NS" -o yaml > "$BACKUP_DIR/host-ingress.yaml" 2>/dev/null || true
