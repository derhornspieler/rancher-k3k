...
# Build kubectl and helm commands with optional --kubeconfig and --context flags.
# Sets KUBECTL_CMD and HELM_CMD global variables.
...
build_kubectl_cmd() {
    ...
    if [[ -n "$context" ]]; then
        ...
        HELM_CMD="$HELM_CMD --kube-context=$context"
    fi
}
...
install_backup_operator() {
    ...
    # Add Helm repo
    $hcmd repo add rancher-charts "$BACKUP_OPERATOR_REPO" --force-update 2>/dev/null || true
    $hcmd repo update rancher-charts 2>/dev/null || true

    # Install CRD chart first
    # shellcheck disable=SC2086
    if ! $hcmd status "$BACKUP_OPERATOR_CRD_CHART" -n "$ns" &>/dev/null; then
        $hcmd install "$BACKUP_OPERATOR_CRD_CHART" "rancher-charts/$BACKUP_OPERATOR_CRD_CHART" \
            -n "$ns" --create-namespace $version_flag
    fi

    # Install operator chart
    # shellcheck disable=SC2086
    if ! $hcmd status "$BACKUP_OPERATOR_CHART" -n "$ns" &>/dev/null; then
        $hcmd install "$BACKUP_OPERATOR_CHART" "rancher-charts/$BACKUP_OPERATOR_CHART" \
            -n "$ns" $version_flag
    fi
...
setup_k3k_kubectl() {
    ...
    HELM_CMD="helm --kubeconfig=$K3K_KUBECONFIG --kube-insecure-skip-tls-verify"
