...
# Fetch a field from a Vault KV v2 secret.
# Usage: VALUE=$(vault_kv_get "kv/services/backup/rancher-backup" "access-key")
# Reads: VAULT_ADDR (defaults to https://vault.example.com)
vault_kv_get() {
    local path="$1"
    local field="$2"
    export VAULT_ADDR="${VAULT_ADDR:-https://vault.example.com}"
    vault kv get -format=json "$path" | jq -r ".data.data[\"${field}\"]"
}
